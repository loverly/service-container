...
Builder.prototype.findServiceJsonFiles = function (absolutePath, level) {
  ...
  // Setup the pattern to find files - the allowJsFiles options makes the json
  // optional and will look for any js or json files.  This will allow for
  // comments and dynamically generated service information
  if (this.options.allowJsFiles) {
    ...
    pattern = new RegExp("services.js(on)?$");
    parameterPattern = new RegExp("parameters.js(on)?$");

    if (this.options.env) {
      envPattern = new RegExp("services_" + this.options.env + ".js(on)?$");
    }
  }
